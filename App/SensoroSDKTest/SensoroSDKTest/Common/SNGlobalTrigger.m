...
@property (nonatomic,strong) NSMutableDictionary * creditsIncrRequestes;
@property (nonatomic,strong) NSMutableDictionary * goodsRequestes;
...
- (void) sendCreditsRequest: (NSString*) sid{
    SNRequest * creditsIncrRequest =
    [SNRequest getPutRequestWithParams:nil
                               delegate:self
                             requestURL:[URLManager creditsIncr:
                                         [SNTopModel sharedInstance].uid
                                                         shopID:sid]];
    if (self.creditsIncrRequestes == nil) {
        self.creditsIncrRequestes = [NSMutableDictionary dictionaryWithCapacity:10];
    }
    [self.creditsIncrRequestes setObject:creditsIncrRequest forKey:sid];
    
    [creditsIncrRequest connect];
}
...
- (void) sendGoodsRequest: (NSString*) sid bid: (NSString*) bid userInfo:(NSDictionary*) dict{
    ...
    SNRequest * goodsRequest = [SNRequest getPutRequestWithParams:params
                                                     delegate:self
                                                   requestURL:[URLManager notifyGoodsInfo:
                                                               [SNTopModel sharedInstance].uid
                                                                                   shopID:sid]];
    goodsRequest.exactInfo = dict;
    
    if (self.goodsRequestes == nil) {
        self.goodsRequestes = [NSMutableDictionary dictionaryWithCapacity:10];
    }
    [self.goodsRequestes setObject:goodsRequest forKey:sid];
    [goodsRequest connect];
}
...
- (void)request:(SNRequest *)request didFailWithError:(NSError *)error
{
    ...
    NSArray * goodsSids = [self.goodsRequestes allKeysForObject:request];
    NSArray * creditsSids = [self.creditsIncrRequestes allKeysForObject:request];
    
    if (creditsSids.count > 0) {
        NSLog(@"credits result failed");
        [self.creditsIncrRequestes removeObjectForKey:[creditsSids objectAtIndex:0]];
    }else if (goodsSids.count > 0){
        NSLog(@"goods result failed");
        [self.goodsRequestes removeObjectForKey:[goodsSids objectAtIndex:0]];
...
- (void)request:(SNRequest *)request didLoad:(id)result
{
    ...
    if (creditsSids.count > 0) {
        ...
        NSString * sid = [dict objectForKey:SID_KEY];
        
        if (sid != nil) {
            SNShop * shop = [[SNTopModel sharedInstance].shopsInfo objectForKey:sid];
            NSString * name = shop.name;
            
            NSString * url = [dict objectForKey:URL_KEY];
            NSDictionary* dict = nil;

            if (url == nil) {
                dict = request.exactInfo;
            }else{
...
- (void) timeTrigger: (SNTrigger*) triger sensor:(SNSensor*) sensors{
    ...
    if([[self.creditsTrigger allKeysForObject:triger] count] > 0){
        ...
        if (model != nil) {
            [self sendCreditsRequest:model.sid];
        }
